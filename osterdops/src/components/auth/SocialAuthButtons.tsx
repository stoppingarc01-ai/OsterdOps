...
export function SocialAuthButtons() {
  ...
  const handleSlackClick = () => {
    setSlackMessage("Slack SSO is available on Enterprise plans. Please use Google or Email.");
    setTimeout(() => setSlackMessage(null), 4000);
  };

  const isBusy = googleLoading || microsoftLoading;
    ...
    <div className="w-full space-y-2">
      <div className="grid grid-cols-3 gap-2.5 w-full">
        ...
        >
          ...
          <span>{microsoftLoading ? "..." : "Microsoft"}</span>
        </button>
      ...
      {slackMessage && (
        <div className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2 text-center animate-in fade-in duration-150">
          {slackMessage}
        </div>
      )}
    </div>
