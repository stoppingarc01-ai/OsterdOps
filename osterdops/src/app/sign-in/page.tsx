...
import { SignInPodiumIllustration } from "@/components/auth/SignInPodiumIllustration";
import { SignInCard } from "@/components/auth/SignInCard";
import { BankGradeSecurityBanner } from "@/components/auth/BankGradeSecurityBanner";
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#07080c] text-white selection:bg-[#dfba82] selection:text-black overflow-x-hidden relative">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 sm:px-10 py-6 lg:py-10 relative z-10 flex flex-col items-center">
        {/* Top Split: Welcome Text (Left) + 3D Podium (Right) */}
        <div className="w-full max-w-[820px] grid grid-cols-1 md:grid-cols-12 gap-6 items-end mb-6">
          ...
          >
            <h1
              className="text-[34px] sm:text-[42px] font-medium tracking-tight text-[#f4efe6] leading-[1.1] flex items-center gap-2.5"
              style={{ fontFamily: "var(--font-serif-luxury), Georgia, serif" }}
            >
              <span>Welcome back</span>
              <span className="text-[30px] sm:text-[36px]">👋</span>
            </h1>
          ...
          >
            <SignInPodiumIllustration />
          </motion.div>
        </div>

        {/* Central Sign-In Auth Card */}
        <motion.div
